...
export default function FeatureList({ imgUrl, title, desc, tags, width, height, gitLink, webLink }) {
    return(
        <div>
            <div className="flex flex-col md:flex-row-reverse w-[80vw] h-max shadow-lg rounded-lg border-2 border-[#B2AC88]">
                ...
                <div className="flex flex-col justify-center items-start p-8 md:w-2/3 h-auto gap-2">
                    <h2 className="text-3xl text-center md:text-start">{title}</h2>
                    <p className="text-center md:text-start">{desc}</p>
                    <Link href={webLink} id="underline" className="flex h-max justify-end items-start">Visit Website</Link>
                </div>
            </div>
        </div>
    )
